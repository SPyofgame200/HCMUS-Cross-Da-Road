...
{
    if ((forced || ptrPlayer->Status().IsMoveLeft()) && ptrPlayer->Physic().CanMoveLeft()) {
        Move(-1, 0, factor);
    }
    return true;
}
...
{
    if ((forced || ptrPlayer->Status().IsMoveRight()) && ptrPlayer->Physic().CanMoveRight()) {
        Move(+1, 0, factor);
    }
    return true;
}
...
{
    if ((forced || ptrPlayer->Status().IsMoveUp()) && ptrPlayer->Physic().CanMoveUp()) {
        Move(0, -1, factor);
    }
    return true;
}
...
{
    if ((forced || ptrPlayer->Status().IsMoveDown()) && ptrPlayer->Physic().CanMoveDown()) {
        Move(0, +1, factor);
    }
    return true;
}
...
{
    if (ptrPlayer->Status().IsMoveRight() && ptrPlayer->IsKilled()) {
    ...
    if (!ptrPlayer->Status().IsMoveLeft() && ptrPlayer->IsKilled()) {
        for (int step = 1; step <= nStep; ++step) {
            MoveLeft(fFactor, true);
            if (!ptrPlayer->IsKilled()) {
                return false;
